...
try:
    from torch_obb import obb_estimate as obb_estimate_warp
    WARP_AVAILABLE = True
except ImportError:
    WARP_AVAILABLE = False


def oriented_bounding_box_pca_torch(vertices: np.ndarray) -> np.ndarray:
    """Compute oriented bounding box using PCA (PyTorch baseline implementation).

    This is a simple PyTorch-based implementation that computes the OBB using
    principal component analysis, matching the behavior of the optimized Warp version.
    ...
    Returns
    -------
    obb_vertices : ndarray, shape (8, 3)
        The 8 corner vertices of the computed oriented bounding box.
    """
    if torch is None:
        raise ImportError("PyTorch is required for this baseline implementation")

    # Convert to torch tensor
    vertices_t = torch.from_numpy(vertices).float()

    if vertices_t.shape[0] == 0:
        return np.zeros((8, 3), dtype=np.float32)

    # Compute mean (center)
    center = torch.mean(vertices_t, dim=0)

    # Compute covariance matrix
    centered = vertices_t - center
    cov = torch.mm(centered.t(), centered) / (vertices_t.shape[0] - 1)
    ...
    projections = torch.mm(centered, eigenvectors)
    min_proj = torch.min(projections, dim=0)[0]
    max_proj = torch.max(projections, dim=0)[0]

    # Half-sizes
    half_sizes = (max_proj - min_proj) * 0.5

    # Create OBB vertices
    # The OBB is axis-aligned in the PCA coordinate system
    # We need to transform the AABB corners back to world space
    ...
    # Scale by half-sizes
    scaled_corners = aabb_corners * half_sizes

    # Rotate and translate to world space
    obb_vertices = torch.mm(scaled_corners, eigenvectors.t()) + center
